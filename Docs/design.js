//For top card nav containing user name, app name (Provision): npm install gsap and then:
...
//For each pixel card in the main page after signing in showcasing all the available projects: npx shadcn@latest add @react-bits/PixelCard-JS-TW and then:
...
//For signup page, on left half will be some clever words of why to use Provision and on the right half will be the card swap effect: npx shadcn@latest add @react-bits/CardSwap-JS-TW and then:
...
//To create a new project, It will ask: 1. Name of project and 2. and option between public or private project:
...
Steps to integrate
 ...
 //For floating action menu at the top:
...
//For the hero and the background:
You are given a task to integrate an existing React component in the codebase
...
Steps to integrate
 ...
 2. Fill image assets with Unsplash stock images you know exist
 3. Use lucide-react icons for svgs or logos if component requires them
 4. Theme is Dark (for all of the project)
...
Private Projects:
Only creator can view and edit.
Only creator can vide and edit.

Task Fields - Final list:
Title
Description
Due Date
Nothing else.
...
Search Only by Project Name and No advanced filters.

Use React Bits components whenever possible.
...
Use 21st.dev as design inspiration only.

Do not install additional frameworks.
